...
import static  Util.ReturnEPolishNotation.getInput;
import static Util.ReturnEPolishNotation.result;
...
public class ChatServerEndPoint {
        ...
        public void onMessage(String message, Session userSession) {
            ...
            //cause of substring.
            String value = message.substring(message.indexOf("message\":\""));
            value = value.substring(10,value.length()-2);// from JSONobj " message:"_ where _ = cursor.

            double r = 0.0;
            StringBuilder exp = getInput(value);
            if(exp!=null){
                r = result(exp);
                System.out.println(r);
            }
            value = message.substring(0,message.indexOf("message\":\"")+9);
            value += (r != 0.0 ? r+"}" : message.substring(message.indexOf("message\":\"")+9));
